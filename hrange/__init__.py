...
class Integer(Step):
    ...
    def to(self, stop, step):
        r = __builtin__.range(self.value, stop.value, step.value)
...
def range(start, stop, step=None, closed=False, string=False):

    if closed:
        # okay, so here's how I want to implement this: 
        # all Step#to methods should return a closed range, 
        # but if closed=False `range` will chop off
        # the last value *if necessary*.
        raise NotImplementedError()

    """
    This function is (or should be) a strict superset of the `range`
    builtin and so can be safely loaded as `from hrange import range`.
    ...
    if start.__class__.__name__ == stop.__class__.__name__:
        if string:
            return [unicode(v) for v in start.to(stop, step)]
        else:
            return [v.value for v in start.to(stop, step)]
    else:
        raise ValueError()
